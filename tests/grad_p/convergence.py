...
def plot_loglog_order_of_convergence(file_path):
    ...
    plt.loglog(sizes, errors, marker='o', linestyle='-', color='b', label='Error')
    ...
    first_order_y = ref_point_y * (sizes / ref_point_x) ** -1
    plt.loglog(sizes, first_order_y, 'r--', label='First-Order Convergence')
    
    # Second-order convergence reference line (slope = -2)
    second_order_y = ref_point_y * (sizes / ref_point_x) ** -2
    plt.loglog(sizes, second_order_y, 'g--', label='Second-Order Convergence')
