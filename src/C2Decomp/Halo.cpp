...
void C2Decomp::updateHalo(C2D_DTYPE *in, C2D_DTYPE *&out, int level, int ipencil) {
    ...
    if (ipencil == 0) { //X Pencil
        ...
        int errorcode = 10;
        std::string errorstring = "Invalid ipencil passed to updateHalo";
    ...
    if (ipencil == 0) {
        ...
        MPI_Type_vector(icount, ilength, ijump, C2D_MPI_DTYPE, &halo12);
        MPI_Type_commit(&halo12);

        //Receive from south
        C2D_DTYPE *southRecvLocation, *northRecvLocation;
        C2D_DTYPE *southSendLocation, *northSendLocation;
        ...
        C2D_DTYPE *bottomRecvLocation, *topRecvLocation;
        C2D_DTYPE *bottomSendLocation, *topSendLocation;
        ...
        MPI_Irecv(bottomRecvLocation, icount, C2D_MPI_DTYPE, neighbor[0][5], tag_b, DECOMP_2D_COMM_CART_X, &requests[0]);
        MPI_Irecv(topRecvLocation, icount, C2D_MPI_DTYPE, neighbor[0][4], tag_t, DECOMP_2D_COMM_CART_X, &requests[1]);
        MPI_Issend(bottomSendLocation, icount, C2D_MPI_DTYPE, neighbor[0][5], tag_b, DECOMP_2D_COMM_CART_X, &requests[2]);
        MPI_Issend(topSendLocation, icount, C2D_MPI_DTYPE, neighbor[0][4], tag_t, DECOMP_2D_COMM_CART_X, &requests[3]);
        MPI_Waitall(4, requests, status);
        ...
        MPI_Type_vector(icount, ilength, ijump, C2D_MPI_DTYPE, &halo21);
        MPI_Type_commit(&halo21);

        C2D_DTYPE *eastRecvLocation, *westRecvLocation;
        C2D_DTYPE *eastSendLocation, *westSendLocation;
        ...
        MPI_Irecv(bottomRecvLocation, icount, C2D_MPI_DTYPE, neighbor[1][5], tag_b, DECOMP_2D_COMM_CART_Y, &requests[0]);
        MPI_Irecv(topRecvLocation, icount, C2D_MPI_DTYPE, neighbor[1][4], tag_t, DECOMP_2D_COMM_CART_Y, &requests[1]);
        MPI_Issend(bottomSendLocation, icount, C2D_MPI_DTYPE, neighbor[1][5], tag_b, DECOMP_2D_COMM_CART_Y, &requests[2]);
        MPI_Issend(topSendLocation, icount, C2D_MPI_DTYPE, neighbor[1][4], tag_t, DECOMP_2D_COMM_CART_Y, &requests[3]);
        MPI_Waitall(4, requests, status);
        ...
        MPI_Type_vector(icount, ilength, ijump, C2D_MPI_DTYPE, &halo31);
        ...
        MPI_Type_vector(icount, ilength, ijump, C2D_MPI_DTYPE, &halo32);
