...
void C2Decomp::writeOne(int ipencil, C2D_DTYPE *var, const std::string& filename) {
    ...
    MPI_Type_create_subarray(3, sizes, subsizes, starts, MPI_ORDER_FORTRAN, C2D_MPI_DTYPE, &new_type);
    ...
    disp = 0;
    MPI_File_set_view(fh, disp, C2D_MPI_DTYPE, new_type, "native", MPI_INFO_NULL);

    MPI_File_write_all(fh, var, subsizes[0] * subsizes[1] * subsizes[2], C2D_MPI_DTYPE, MPI_STATUS_IGNORE);
...
void C2Decomp::readOne(int ipencil, C2D_DTYPE *var, const std::string& filename) {
    ...
    MPI_File_read_all(fh, var, subsizes[0] * subsizes[1] * subsizes[2], C2D_MPI_DTYPE, MPI_STATUS_IGNORE);
...
void C2Decomp::writeVar(MPI_File &fh, MPI_Offset &disp, int ipencil, C2D_DTYPE *var) {
...
void C2Decomp::readVar(MPI_File &fh, MPI_Offset &disp, int ipencil, C2D_DTYPE *var) {
...
void C2Decomp::writeScalar(MPI_File &fh, MPI_Offset &disp, int n, C2D_DTYPE *var) {

    int m;

    MPI_File_set_view(fh, disp, C2D_MPI_DTYPE, C2D_MPI_DTYPE, "native", MPI_INFO_NULL);
    ...
    MPI_File_write_all(fh, var, m, C2D_MPI_DTYPE, MPI_STATUS_IGNORE);

    disp += n * myTypeBytes;
}

void C2Decomp::readScalar(MPI_File &fh, MPI_Offset &disp, int n, C2D_DTYPE *var) {

    MPI_File_set_view(fh, disp, C2D_MPI_DTYPE, C2D_MPI_DTYPE, "native", MPI_INFO_NULL);

    MPI_File_read_all(fh, var, n, C2D_MPI_DTYPE, MPI_STATUS_IGNORE);

    disp += n * myTypeBytes;
}

void C2Decomp::writePlane(int ipencil, C2D_DTYPE *var, int iplane, int n, const std::string& filename) {
    ...
    if (iplane == 0) {
        ...
        wk2d = new C2D_DTYPE[decompMain.xsz[1] * decompMain.xsz[2]];
        ...
        wk2d = new C2D_DTYPE[decompMain.ysz[0] * decompMain.ysz[2]];
        ...
        wk2d = new C2D_DTYPE[decompMain.zsz[0] * decompMain.zsz[1]];
    ...
    disp = 0;
    MPI_File_set_view(fh, disp, C2D_MPI_DTYPE, new_type, "native", MPI_INFO_NULL);
    MPI_File_write_all(fh, wk2d, subsizes[0] * subsizes[1] * subsizes[2], C2D_MPI_DTYPE, MPI_STATUS_IGNORE);
