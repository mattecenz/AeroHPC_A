...
void poissonSolver::performFFT()
{
    ...
    for (int k = 0; k < xSize[2]; k++){
        for (int j = 0; j < xSize[1]; j++){
            const int base_index = xSize[0] * xSize[1] * k + xSize[0] * j;
            memcpy(fft_inputX, &base_buffer[base_index], sizeof(Real) * xSize[0]);
            fftwr_execute(fftwr_plan_r2r_1d(xSize[0], fft_inputX, fft_outputX, FFTW_REDFT10, FFTW_ESTIMATE)); // Execute FFT on each slice
            memcpy(&base_buffer[base_index], fft_inputX, sizeof(Real) * xSize[0]);
        }
    }
    ...
    for (int i = 0; i < ySize[0]; i++){
        for (int k = 0; k < ySize[2]; k++){
            const int base_index = ySize[2] * ySize[1] * i + ySize[1] * k;
            memcpy(fft_inputY, &u2[base_index], sizeof(Real) * ySize[1]);
            fftwr_execute(fftwr_plan_r2r_1d(ySize[1], fft_inputY, fft_outputY, FFTW_REDFT10, FFTW_ESTIMATE)); // Execute FFT on each slice
            memcpy(&u2[base_index], fft_inputY, sizeof(Real) * ySize[1]);
        }
    }
    ...
    for (int j = 0; j < zSize[1]; j++){
        for (int i = 0; i < zSize[0]; i++){
            const int base_index = zSize[2] * zSize[0] * j + zSize[2] * i;
            memcpy(fft_inputZ, &u3[base_index], sizeof(Real) * zSize[2]);
            fftwr_execute(fftwr_plan_r2r_1d(zSize[2], fft_inputZ, fft_outputZ, FFTW_REDFT10, FFTW_ESTIMATE)); // Execute FFT on each slice
            memcpy(&u3[base_index], fft_inputZ, sizeof(Real) * zSize[2]);
        }
    }
}
...
void poissonSolver::performIFFT()
{
    ...
    for (int j = 0; j < zSize[1]; j++) {
        for (int i = 0; i < zSize[0]; i++) {
            const int base_index = zSize[2]*zSize[0] * j + zSize[2] * i;
            memcpy(fft_inputZ, &u3[base_index], sizeof(Real) * zSize[2]);
            fftwr_execute(fftwr_plan_r2r_1d(zSize[2], fft_inputZ, fft_outputZ, FFTW_REDFT01, FFTW_ESTIMATE));
            memcpy(&u3[base_index], fft_inputZ, sizeof(Real) * zSize[2]);
        }
    }
    ...
    for (int i = 0; i < ySize[0]; i++) {
        for (int k = 0; k < ySize[2]; k++) {
            const int base_index = ySize[2]*ySize[1] * i + ySize[1] * k;
            memcpy(fft_inputY, &u2[base_index], sizeof(Real) * ySize[1]);
            fftwr_execute(fftwr_plan_r2r_1d(ySize[1], fft_inputY, fft_outputY, FFTW_REDFT01, FFTW_ESTIMATE));
            memcpy(&u2[base_index], fft_inputY, sizeof(Real) * ySize[1]);
        }
    }
