...
namespace utils
{
    ...
    */
    template <Addressing_T A, Component to, Component from>
    inline Real get_interpolation(const StaggeredGrid<A> &grid, int i, int j, int k);
    ...
    */
    template <Addressing_T A, Component c>
    inline Real conv(const Model<A> &model, int i, int j, int k);
}
#endif